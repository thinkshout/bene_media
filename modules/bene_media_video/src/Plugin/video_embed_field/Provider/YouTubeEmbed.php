...
namespace Drupal\bene_media_video\Plugin\video_embed_field\Provider;

use Drupal\video_embed_field\ProviderPluginBase;

/**
 * A YouTube Embedded provider plugin.
 *
 * @VideoEmbedProvider(
 *   id = "youtubeembed",
 *   title = @Translation("YouTube Embed")
 * )
 */
class YouTubeEmbed extends ProviderPluginBase {
  /**
   * {@inheritdoc}
   */
  ...
  public static function getIdFromInput($input) {
    preg_match('%(?:youtube(?:-nocookie)?\.com/(?:[^/]+/.+/|(?:v|e(?:mbed)?)/|.*[?&]v=)|youtu\.be/)([^"&?/ ]{11})%i', $input, $matches);
